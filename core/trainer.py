...
if __name__ == "__main__":
	...
	# X, y = load_circles(n_samples=1000,noise=0.1).load()

	model = Model()
	model.add(DenseLayer(2, 16, He(), "relu", regularizer=None))
	model.add(DenseLayer(16, 8, He(), "relu", regularizer=None))
	model.add(DenseLayer(8, 1, Xavier(), "sigmoid", regularizer=None))
	...
	print(f"\nFinal loss: {final_metrics['loss']:.4f}")
	print(f"\nFinal accuracy: {final_metrics['accuracy']:.2%}")
	print(f"\nFinal precision: {final_metrics['precision']:.2%}")
	print(f"\nFinal recall: {final_metrics['recall']:.2%}")
	print(f"\nFinal f1: {final_metrics['f1']:.2%}")
